...
public class GameManager : MonoBehaviour
{
    ...
    // 【修改】將語音播放邏輯從 StartCoroutine(WaitForAnswer) 中分離
    IEnumerator FishStallQuestionSequence()
    {
        ...
        yield return StartCoroutine(WaitForAnswer(new List<string> { "魚", "魚肉", "魚攤", "肉", "海鮮", "魚肉攤" }));

        // 第二個語音問題
        Debug.Log("Console 問題: 魚的顏色是什麼？");
        yield return StartCoroutine(PlayAudioClipAndThenWait(fishColorAudioClip));
        yield return StartCoroutine(WaitForAnswer(new List<string> { "藍色", "藍", "藍白", "藍白色", "淺藍", "淺藍色" }));

        // 第三個語音問題 (帶圈圈)
        Debug.Log("Console 問題: 那個是什麼？");
        yield return StartCoroutine(PlayAudioClipAndThenWait(whatIsThatAudioClip));
        ShowHighlightCircle();
        yield return StartCoroutine(WaitForAnswer(new List<string> { "燈", "路燈" }));
        HideHighlightCircle();

        Debug.Log("Console: 所有魚攤問題已完成！");
    }

    // 【新增】一個專門用於播放音訊並等待其播放完畢的協程
    IEnumerator PlayAudioClipAndThenWait(AudioClip clip)
    {
        ...
        // 核心修改：將音訊指定給 voiceAudioSource.clip
    ...
    // 【修改】此函式改為 PlayOneShot，只用於點擊題目的語音播放
    ...
    {
        // 核心修改：移除原有的等待音訊播放時間的程式碼
        // 因為現在我們在呼叫這個協程前已經先等待音訊播放完畢了

        Debug.Log("請說出你的答案...");
        questionBroadcastTextMeshPro.text = "請說出你的答案...";
        ...
        else
        {
            ...
            questionBroadcastTextMeshPro.text = "沒有找到麥克風設備！";
    ...
    IEnumerator SendAudioToServer(byte[] audioData, List<string> correctAnswers)
    {
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                questionBroadcastTextMeshPro.text = "辨識失敗，請再試一次。";
            }
        }
        else
        {
            Debug.LogError("語音辨識請求失敗: " + request.error);
            questionBroadcastTextMeshPro.text = "網路錯誤或伺服器問題。";
        }
    }
    ...
    void CheckAnswer(string userResponse, List<string> correctAnswers)
    {
        ...
        {
            Debug.Log("沒有聽到回答。");
            questionBroadcastTextMeshPro.text = "沒有聽到回答。";
            return;
        }

        bool isCorrect = false;
        // 將使用者回答正規化為小寫，並去除前後空白
        ...
        foreach (string correctAnswer in correctAnswers)
        {
            ...
            // 核心修改：判斷方式從 Equals 改為 Contains，以增加彈性
        ...
        if (isCorrect)
        {
            ...
            questionBroadcastTextMeshPro.text = "答案正確！";
        }
        else
        {
            Debug.Log($"答案錯誤。你說了: \"{userResponse}\"，正確答案是: \"{string.Join("/", correctAnswers)}\"");
            questionBroadcastTextMeshPro.text = $"答案錯誤。";
        }
