...
{
    // ----- 刪除 -----
    // private FirebaseManager_Firestore FirebaseManager; // 註解掉，請確保您有其他方式獲取 FirebaseManager_Firestore.Instance
    ...
    // ----- 刪除 -----
    // public Button confirmButton; // 您的程式碼已經沒有 confirmButton 了，保持刪除
    ...
    void InitializeTest()
    {
        ...
        HideAllPanels();
        // ----- 刪除 -----
        // if (startPanel != null) startPanel.SetActive(true); // 不再需要顯示開始畫面
        
        // if (confirmButton != null) confirmButton.gameObject.SetActive(false); // 已刪除
    ...
    public void RecordSelection(string selection)
    {
        ...
        {
            int selectedHour = Parse12HourFormat(selection);
        ...
        {
            isCorrect = (selection == correctAnswers[currentQuestionKey]);
        }

        playerAnswers[currentQuestionKey] = selection;
        ...
        if (IsSceneFinished())
        {
            ...
            // 確保 FirebaseManager_Firestore.Instance 是可訪問的
            if (FirebaseManager_Firestore.Instance != null)
            {
            ...
            {
                Debug.LogError("[GM] FirebaseManager_Firestore.Instance 為 null！無法儲存分數。");
            }
    ...
    #region Unchanged Data Structures & Save Logic
    [Serializable]
    private class QuestionRecord { public string Key; public string CorrectAnswer; public string PlayerAnswer; }
    [Serializable]
    private class ResultData { public string PlayerId; public string Timestamp; public List<QuestionRecord> Questions; public int TotalCorrect; }
    ...
    #region Public Control Methods (不變)
    ...
    #region Scenery & Season Methods (不變)
